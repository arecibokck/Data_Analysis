...
QuarterPeriodSpreads = {};
QuarterPeriods = {};
for GroundStatePopulation = [0.3 0.7 0.9]
    ...
    SpreadAtQuarterPeriod = zeros(size(InitialSpreads, 1),1);
    QuarterPeriod = zeros(size(InitialSpreads, 1),1);
    for ii = 1:length(InitialSpreads)
        
        ProbDF  = GaussianDistribution(0, (InitialSpreads(ii) * 1e-6)^2, positions);
        ...
        [SpreadAtQuarterPeriod(ii), idx] = min(RMSSpread);
        QuarterPeriod(ii) = tspan(idx);
    end
    QuarterPeriodSpreads{end+1} = SpreadAtQuarterPeriod;
    QuarterPeriods{end+1} = QuarterPeriod;
end
%%
plotQuarterPeriodSpreads(InitialSpreads, QuarterPeriodSpreads, QuarterPeriods, NumberOfAtoms, FractionOfInitialPotential, [0.3 0.7 0.9])
%% Plotting functions
