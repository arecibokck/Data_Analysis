...
VelocityType = 'Mean'; % 'Mean' | 'RMS'

QuarterPeriods = {};
for GroundStatePopulation = 0.3:0.2:1.1
    if GroundStatePopulation < 1
        ...
        switch VelocityType
            case 'Mean'
                Velocity = - sqrt((2*BoltzmannConstant*initialTemperature)/Cs133Mass) * gamma(1)/gamma(1/2);
            case 'RMS'
                Velocity = - sqrt((BoltzmannConstant*initialTemperature)/Cs133Mass); %RMS
        end
    else
        Velocity = 0;
    end    
    ...
    initialPositions = (2:0.5:25).*1e-06;
    QuarterPeriod = zeros(size(initialPositions, 1),1);
    for Index = 1:length(initialPositions)
        Xres = zeros(length(tspan),1);
        Vres = zeros(length(tspan),1);
        progressbar  = parforNotifications();
        progressbar.PB_start(1,'Message',['Computing evolution for ' num2str(1,'%.0f') ' atoms:']);
        InitialConditions = [initialPositions(Index) Velocity]; % [initial position, initial velocity]
        [res] = ode5(@(t, x) odefcn(t, x, Trap, PotentialType), tspan, InitialConditions);
        Xres(:,Index) = res(:,1);
        Vres(:,Index) = res(:,2);
        progressbar.PB_iterate();
        RMSSpread = zeros(size(Xres, 1),1);
        for ii = 1:size(Xres, 1)
            RMSSpread(ii) = rms(Xres(ii,:));
        end
        [~, idx] = min(RMSSpread);
        QuarterPeriod(Index) = tspan(idx);
    end
    QuarterPeriods{end+1} = QuarterPeriod;
end
%%
plotQuarterPeriodsForRMSVelocity(Trap, initialPositions, QuarterPeriods, 0.3:0.2:1.1)
%% Plotting functions
...
function plotQuarterPeriodSpreads(InitialSpreads, QuarterPeriodSpreads, QuarterPeriods, NumberOfAtoms, FractionOfInitialPotential, GroundStatePopulation)
    figure(11)
    clf
    %colours = {[0, 0.4470, 0.7410],[0.8500, 0.3250, 0.0980],[0.9290, 0.6940, 0.1250],[0.4940, 0.1840, 0.5560],[0.4660, 0.6740, 0.1880], [0.6350, 0.0780, 0.1840]};
    %set(gcf, 'defaultAxesColorOrder', [colours{1};colours{5}]);
    ...
    set(gcf, 'OuterPosition', [0.5 0.5 0.25 0.45]);
    Markers = {'o', '+', 'v', 's', 'p', 'h', 'd', '^'};
    for Index = 1:size(QuarterPeriodSpreads, 2)
        plot(InitialSpreads, QuarterPeriodSpreads{Index}.*1e6, strcat('--', Markers{Index}), 'MarkerSize', 5)
        hold on
    end
    xlabel('Initial Spread [\mum]','FontSize', 14)
    ylabel('Quarter Period Spread [\mum]','FontSize', 14)
    legend(strcat(['#:' num2str(NumberOfAtoms) '; P_{GS}:'], strsplit(num2str(100.*GroundStatePopulation)), '%; ', ['U_{release}:' num2str(100*FractionOfInitialPotential) '%']),'FontSize', 14)
    grid on
%     yyaxis right
%     for Index = 1:size(QuarterPeriodSpreads, 2)
%         plot(InitialSpreads, QuarterPeriods{Index}.*1e3, strcat('--', Markers{Index}), 'MarkerSize', 5)
%         hold on
%     end
    sgtitle('Spread at Quarter Period v Initial Spread of atoms');
end
function plotQuarterPeriodsForRMSVelocity(Trap, initialPositions, QuarterPeriods, GroundStatePopulation)
    figure(12)
    clf
    %colours = {[0, 0.4470, 0.7410],[0.8500, 0.3250, 0.0980],[0.9290, 0.6940, 0.1250],[0.4940, 0.1840, 0.5560],[0.4660, 0.6740, 0.1880], [0.6350, 0.0780, 0.1840]};
    %set(gcf, 'defaultAxesColorOrder', [colours{1};colours{5}]);
    ...
    for Index = 1:size(QuarterPeriods, 2)
        plot((initialPositions ./ Trap.w0), (QuarterPeriods{Index} ./ HarmonicQuarterPeriod), strcat('--', Markers{Index}), 'MarkerSize', 5)
        hold on
    end
    ylim([0 2])
