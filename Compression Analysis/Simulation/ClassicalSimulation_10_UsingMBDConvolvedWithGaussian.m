...
FractionOfInitialPotential = 0.6;
...
NumberOfAtoms = 1000;
% initialPositions  = (randn(NumberOfAtoms,1) + 1).* 20e-6;
initialPositions  = randn(NumberOfAtoms,1).* 20e-6;
max_vel = 2e-3;
velocities = 0:(max_vel/(NumberOfAtoms-1)):max_vel;
InitialTrapDepth = LatticeProperties.estimateTrapDepthFromHeatingSidebandFreqDT1DT3(LongitudinalTrapFrequency);
...
ProbDF    = conv(MaxwellBoltzmannDistribution(initialTemperature, velocities), GaussianDistribution(0, DeltaPQuantumMechanical, velocities));
initialVelocities = drawSamplesFromDistribution(NumberOfAtoms, 0:max_vel/(length(ProbDF)-1):max_vel, ProbDF);
...
if DebugMode
    PlotPotential                                  = false;
    PlotSampling                                   = false;
    PlotConvolvedDistributionSampling              = true;
    ...
    PlotPotential                                  = false;
    PlotSampling                                   = false;
    PlotConvolvedDistributionSampling              = false;
    PlotTrajectories                               = false;
    PlotEnvelopesAndOptimalWaitingTimes            = false;
    PlotPhaseSpaceEvolution                        = false;
    PlotRMSpread                                   = true;
...
if PlotConvolvedDistributionSampling
    plotConvolvedDistributionSampling(NumberOfAtoms, initialTemperature, velocities)
end
if PlotTrajectories
