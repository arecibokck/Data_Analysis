...
initialPositions  = [24; 26; 28; 34; 36; 38; 44; 46; 48]*1e-6;
NumberOfAtoms = length(initialPositions);
initialVelocities = [1e-4; 1e-3; 7e-2; 1e-4; 1e-3; 5.45e-2; 1e-4; 1e-3; 4e-2];
%initialVelocities = linspace(0, 1e-3, NumberOfAtoms);
if ~DebugMode
...
function plotPhaseSpaceEvolution(NumberOfAtoms, GroundStatePopulation, initialTemperature, tNumPoints, Xres, Vres, NumberOfBins)
    figure(7)
    clf
    set(gcf, 'Units', 'normalized');
    set(gcf, 'OuterPosition', [0.5 0.5 0.25 0.45]);
    ...
    for Time = 1:50:tNumPoints
        ...
        sb1 = subplot(4,4,[1,9]);
        h1 = histfit(velocities(:), NumberOfBins);
        h1(1).FaceAlpha =  0.1;
        h1(1).FaceColor = colours{2};
        h1(2).Color     = colours{4};
        h1(2).LineStyle = '--';
        set(sb1, 'Box', 'off', 'Color', 'none')
        set(sb1, 'xlim', [-round(max(max(Vres))*1e3, 1) round(max(max(Vres))*1e3, 1)])
...
%       colormap(jet(size(velocities,2)))
        ...
        h2 = histfit(positions(:), NumberOfBins);
        h2(1).FaceAlpha =  0.1;
        h2(1).FaceColor = colours{2};
        h2(2).Color     = colours{4};
        h2(2).LineStyle = '--';
        set(sb3, 'Box', 'off', 'Color', 'none')
        set(sb3, 'xlim', [-round(max(max(Xres))*1e6, 1) round(max(max(Xres))*1e6, 1)])
