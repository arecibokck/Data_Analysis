...
workingDir = pwd;
endstring = strcat(datasetname(5:end));
new_Folder_name = strcat(endstring(1:4),'-', endstring(5:6), '-',endstring(7:8), '-', endstring(9:end));
...
for n = 1:n_run
    all_pos = {};
    for i = 1:sizeimages %Iterate through number of images in each run
    ...
    try
        ...
        fprintf('Skipping due to excessive combinitorics...\n');
    end
    ...
    for i = 1:max(result, [], 4) %iterate through all identified trajectories, the tag is a number and the maximum is the total number of identified trajectories 
        ...
        if (length(indr) == sizeimages) %Filters only complete trajectories 
            for j = 1:length(indr)
                x = [x result(indr(j),1)];
    ...
    if t > 1
        ...
        filename = strcat(datasetname(5:end),'_n_',int2str(n),'_Atom_Transport_Tracked.avi');
        Dir = strcat(workingDir, filesep, new_Folder_name);

        %Plot tracks of atoms on images and create a video
        outputVideo = VideoWriter(fullfile(Dir, filename), 'Uncompressed AVI');
        ...
        fprintf('No regular trajectory(ies) found. Video rendering abandoned.\n');
    end
end